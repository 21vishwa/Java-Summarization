...
java_folder_path = '/content/drive/MyDrive/java file'
...
class CodeProcessor:
    ...
    def generate_node_features(self, blocks, detailed_summaries):
        node_features = {}

        for class_name, class_info in blocks.items():
            class_code = class_info['code']
            class_embedding = self.embedding_model.encode(class_code)
            num_code_lines = len(class_code.splitlines())
            methods = re.findall(r'(public|private|protected)?\s*(static)?\s*\w+\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\([^)]*\)\s*\{', class_code)
            num_methods = len(methods)
            has_constructor = 1 if any(method[2] == class_name for method in methods) else 0
            summary_embedding = self.embedding_model.encode(detailed_summaries[class_name])

            node_features[class_name] = {
    ...
    def generate_adjacency_matrix(self, summaries):
        class_names = list(summaries.keys())
        adjacency_matrix = np.zeros((len(class_names), len(class_names)))

        for i, class_name1 in enumerate(class_names):
            for j, class_name2 in enumerate(class_names):
                if i == j:
                    adjacency_matrix[i][j] = 1  # Diagonal set to 1
                else:
                    similarity = self.compute_similarity(summaries[class_name1], summaries[class_name2])
    ...
    def process_files(self, java_files, similarity_threshold=0.5):
        ...
        # Collect relationships across all classes in all components
            node_features = self.generate_node_features(blocks, detailed_summaries)
        ...
        adjacency_matrix = self.generate_adjacency_matrix(all_summaries)

        # Print node features and adjacency matrix
...
node_features, adjacency_matrix = code_processor.process_files(java_files)
